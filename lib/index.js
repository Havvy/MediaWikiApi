...
MediaWikiApi.prototype = {
    ...
    categorymembers: function (category, options, callback) {
        ...
        for (var option in options) {
            cmoptions['cm' + option] = options[option];
        }
    ...
    edit: function (title, body, callback) {
        ...
        // Handle ensuring the text is sent properly.
        var md5 = crypto.createHash('md5')
        if (body.appendtext || body.prependtext) {
            md5.update((body.prependtext || "") + (body.appendtext || ""));
        } else if (body.text) {
            md5.update(body.text);
        } else {
        ...
        this._getToken('edit', function (err, token) {
            ...
            // Handle making the body a String.
    ...
    _request: function (options, callback, method, body) {
        ...
        request(req).when(function (err, ahr, data) {
            ...
            if (ahr.headers['mediawiki-api-error']) {
                if (ahr.headers['mediawiki-api-error'] === 'help') {
                    // TODO: Give a link to Wikipedia's default help page.
                    callback({ 
                        code: 'help', 
    ...
    _post: function (options, body, callback) {
        this._request(options, callback, 'POST', body);
    },

    _get: function (options, callback) {
        this._request(options, callback, 'GET');
    },
